...
/*
<option value="R320">Apple Store, Sanlitun</option>
<option value="R448">Apple Store, Wangfujing</option>
<option value="R388">Apple Store, Xidan Joy City</option>
<option value="R484">Apple Store, Holiday Plaza Shenzhen</option>
<option value="R390">Apple Store, Hong Kong Plaza</option>
<option value="R359">Apple Store, Nanjing East</option>
<option value="R389">Apple Store, Pudong</option>
*/

//MD297ZP/A
var skus = new Array("MD297CH/A", "MD298CH/A");

var locale = 'zh_CN';

function generate() {
	console.log("#!/usr/bin/python");
	console.log("#-*-coding:utf-8-*-\n\nkeys = [");

	var fs = require("fs");
	var lines = fs.readFileSync(userlist, "utf8").split('\n');
	for(var i=0; i<lines.length; i++)
	{
		var tokens = lines[i].split(' ');
		if(tokens.length != 3) {
			console.error("error line:", lines[i]);
			continue;
		}

		cart.firstName = tokens[0].substring(1);
		cart.lastName = tokens[0].substring(0, 1);
		cart.governmentID = tokens[1];
		cart.emailAddress = tokens[2];
		...
		var payload = cart.governmentID + ',' + locale
				+ ',' + cart.storeNumber + ','
				+ cart.sku + ','
				+ cart.quantity + ','
				+ cart.firstName + ','
				...
				+ cart.planType;
		if(i==0) {
			//"123456789098765432,zh_CN,R502,MD300CH/A,5,新利,陈,chen.daqi@gmail.com,undefined,iPhone,UNLOCKED"
            console.error("413024198211113811,zh_CN,R390,MD297CH/A,5,新利,陈,chen.daqi@gmail.com,undefined,iPhone,UNLOCKED")
			console.error(payload);
		}
		payload = encodeToUTF8(payload);
	    	
		var key = CryptoJS.HmacSHA256(cart.governmentID + cart.productName, hmacKey);
		var encryptedPayload = doEncrypt(pgpId, 0, pgpKey, payload);
		var data = 'key=' + key + '&payload=' + encryptedPayload;
		console.log("'''"+data+"''',");
	}
...
/********************************************/
userlist = process.argv[2];
store = process.argv[3];
whiteratio = 0.6;

var cart = new Object();
cart.productName = 'iPhone';
cart.storeNumber = store;
cart.sku = 'MD531CH/A';		// MD128CH/A, MD198CH/A
cart.quantity = '5';		// 1, 2, 3, 4, 5
cart.firstName = '';
cart.lastName = '';
cart.emailAddress = '';
cart.governmentID = '';
cart.planType = 'UNLOCKED';
